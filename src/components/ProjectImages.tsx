	...
	return (
		<picture>
			<For each={options().options} fallback={null}>
				{(item, index) => <source sizes={item.sizes} srcset={options().srcSet[index()].attribute} type={`image/${item.format}`} />}
			</For>
			...
			<dialog class="modal" ref={modalRef}>
				<div class="modal-box w-auto max-w-full p-0 md:max-w-10/12">
					<Show when={selectedImage()} fallback={<div>Loading...</div>}>
						{(item) => <Picture options={item()} alt="Project's UI mockup" class="size-full max-h-screen object-contain" />}
					</Show>
				</div>
